...
async def generate_content(request: ContentRequest):
    try:
        content = await agent.generate_action_content(
            request.action_type, request.action_text
        )
...
async def get_benchmark(domain: str = Query(...)):
    try:
        ...
        report_data = (
            report.get("data", [{}])[0]
            if isinstance(report.get("data"), list)
            else report.get("data", {})
        )
        ...
        top_slices = sorted(
            slices, key=lambda x: x.get("opportunity_score", 0), reverse=True
        )[:5]
        ...
        for s in top_slices:
            ...
            priority_score = s.get(
                "relative_opportunity_score", s.get("opportunity_score", 0)
            )
        ...
        own_domain = (
            own_brand.get("domain", clean_domain) if own_brand else clean_domain
        )
        ...
        for ch_name in channel_names:
            ...
            if ch_domain:
                ch_slice = next(
                    (s for s in slices if ch_domain in s.get("domain", "")), None
                )
            else:
                ch_slice = next(
                    (
                        s
                        for s in slices
                        if "EDITORIAL" in s.get("action_group_type", "").upper()
                        or "LISTICLE" in s.get("url_classification", "").upper()
                    ),
                    None,
                )

            if ch_slice:
                ch_slice.get("gap_percentage", 0)
            ...
            for cb in competitor_brands:
                ...
                cb_data = (
                    cb_report.get("data", [{}])[0]
                    if isinstance(cb_report.get("data"), list)
                    else cb_report.get("data", {})
                )
                cb_vis = cb_data.get(
                    "retrieved_percentage", cb_data.get("visibility_score", 0)
                )
                comp_data.append(
                    {
                        "name": cb.get("name", cb.get("domain", "")),
                        "visibility": round(cb_vis, 1),
                    }
                )

            own_report = await peec.get_domain_report(own_domain)
            own_data = (
                own_report.get("data", [{}])[0]
                if isinstance(own_report.get("data"), list)
                else own_report.get("data", {})
            )
            own_vis = own_data.get(
                "retrieved_percentage", own_data.get("visibility_score", 0)
            )
        ...
        for tab, cfg in tab_map.items():
            if "domain" in cfg:
                cat_slice = next(
                    (s for s in slices if cfg["domain"] in s.get("domain", "")), None
                )
            else:
                cat_slice = next(
                    (
                        s
                        for s in slices
                        if cfg["type"] in s.get("action_group_type", "").upper()
                        or cfg["classification"]
                        in s.get("url_classification", "").upper()
                    ),
                    None,
                )
            ...
            if cat_slice:
                if "domain" in cfg:
                    details = await peec.get_actions(
                        pid, scope=cfg["scope"], domain=cfg["domain"]
                    )
                else:
                    details = await peec.get_actions(
                        pid,
                        scope=cfg["scope"],
                        url_classification=cfg["classification"],
                    )
        ...
        for _tab, data in tab_actions.items():
        ...
        return {
            ...
            "estimated": {
                "visibility_score": target_visibility,
                "citation_count": target_citations,
            },
            "roadmap": roadmap,
